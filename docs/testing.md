...
## From-zero setup proof (2026-09-09)

A pristine `debian:bookworm-slim` image installed Kelpie with
`cargo install kelpie-herdr --version 0.2.0-alpha.4`, then built the current
nostrherd working tree. Bare `cargo install kelpie-herdr` failed because it
selected the yanked stable release instead of a live alpha.

The fresh runtime ran `nostrherd init /work/bots/mybot --id mybot --kind pi`,
`nostrherd --check`, and finally `nostrherd` with no flags or XDG overrides.
The registry and database used their conventional paths. The relocated binary
had the shipped conduct skill beside it; generated `AGENTS.md` was unchanged.
...
The loopback `groups_relay` fixture needed a disposable build-only
`recursion_limit = "256"` adjustment under Rust 1.98.1. Neither nostrherd nor
Kelpie needed a fixture patch. This proves one live first reply, not every
SPEC flow or compatibility with every agent and relay version.
...
a tell body publishes whole, markup and all (`occupant_tell_publishes_any_text_including_angle_brackets`).
...
filter replacement is covered by `subscription_refresh_replaces_all_dynamic_filters`.
