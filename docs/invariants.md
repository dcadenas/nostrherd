...
| I18 | A host wake MUST carry a typed section, queue behind open work, and publish its final once through the normal stamped path without a fake reply or mention | `host_watch_fire_opens_a_normal_turn_with_a_typed_section`, `host_watch_fire_queues_behind_an_open_turn`, `host_wake_final_publishes_without_a_reply_or_mention` |
| I19 | Watched-author matching MUST run in the host, record before wake, fire once per cooldown, ignore unwatched authors, and refresh author subscription scope when watches change | `watched_author_fires_once_per_cooldown_and_unwatched_author_does_not`, `watch_scope_lifetime_and_cancel_control_author_subscription`, `subscription_refresh_replaces_all_dynamic_filters`, `subscription_refresh_gate_covers_scope_errors_and_idle` |
