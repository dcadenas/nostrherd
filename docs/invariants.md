# Invariants

Each MUST names the test that currently proves it. SPEC still wins if
this file and SPEC disagree.

| # | Invariant | Test |
| --- | --- | --- |
| I1 | Two bots MUST NOT share a session name | `session_name_tries_more_of_uuid_after_a_prefix_collision`, `session_names_disambiguate_when_bot_and_display_collide`, `session_names_include_the_bot_id` |
| I2 | A session for place A MUST NOT be given history from place B | `snapshot_omits_other_channels_including_dms`, `later_ask_includes_unread_same_channel_delta` |
| I3 | Replaying a processed `EventId` MUST NOT open a second turn | `processed_events_are_idempotent` |
| I4 | Kelpie `from=` for this host is only `nostrherd` | `names_socket_waiter_nostrherd`, `ask_is_owned_by_waiter_and_passes_body_on_stdin` |
| I5 | A trigger is first token `{bot-id}:` after an optional mention, plus either an operator `p`-tag or operator authorship | `trigger_requires_operator_p_tag_unless_operator_authored`, `trigger_allows_one_leading_mention`, `trigger_rejects_non_prefix_and_inexact_tokens`, `operator_authored_bot_colon_without_self_p_tag_emits`, `peer_authored_bot_colon_without_operator_p_tag_is_indexed_only`, `inbound_trigger_token_is_the_configured_bot_id`, `inbound_tokens_route_to_the_matching_bot`, `inbound_tokens_queue_separate_sessions_for_each_bot`, `action_bot_id_follows_the_active_turn_for_edits`, `actor_for_bot_selects_the_matching_actor`, `bot_uses_id_as_inbound_trigger_and_rejects_empty_kind` |
| I6 | An empty request after `bot:` MUST NOT open a Turn | `empty_trigger_request_is_not_asked` |
| I7 | Turn states are parsed tokens; there is no `publishing` state | `turn_state_parses_known_tokens_only` |
| I8 | Only queued→open/failed/cancelled and open→posted/failed/cancelled are legal | `turn_transition_parses_legal_changes_only`, `turns_queue_in_order_and_state_changes_are_terminal`, `dispatch_failure_stamps_once_and_fails_only_queued_turns` |
| I9 | A claimed open turn MUST NOT be cancelled | `claimed_open_turn_cannot_be_cancelled`, `flow_10_claimed_turn_keeps_the_landing_reply` |
| I10 | The host MUST NOT publish on a cancelled ask | `cancelled_turn_acks_without_publish`, `spec_flow_10_cancelled_turn_never_reaches_buzz` |
| I11 | Binaries MUST read `NOSTRHERD_PRIVATE_KEY` and `NOSTRHERD_RELAY_URL` when set, and MUST NOT take `--envchain` | `operator_env_reads_buzz_private_key_and_relay_url`, `runtime_requires_operator_key_from_env`, `runtime_requires_relay_url_from_env`, `nostrherd_parser_rejects_envchain` |
| I12 | SQLite MUST NOT store nsecs | `sqlite_does_not_persist_an_nsec`, `sqlite_schema_has_no_nsec_columns` |
| I13 | Host MUST add `⏳` on queued/open and remove it when work on that EventId ends; an edit that re-queues the same EventId keeps the marker; occupants MUST NOT react | `trigger_adds_in_flight_reaction`, `queued_trigger_adds_its_own_in_flight_reaction`, `delete_removes_in_flight_reaction`, `occupant_final_removes_in_flight_reaction`, `late_final_on_edited_trigger_keeps_in_flight_reaction`, `posted_turn_removes_in_flight_reaction`, `failed_turn_removes_in_flight_reaction`, `progress_does_not_remove_in_flight_reaction` |
| I14 | Host stamp is `**[{bot-id}]**:`; occupants MUST NOT stamp; own stamped posts MUST NOT retrigger | `stamp_outbound_prefixes_once`, `bot_uses_id_as_inbound_trigger_and_rejects_empty_kind`, `pr_bot_final_publishes_pr_stamp_once`, `final_with_text_publishes_then_acks`, `stamped_self_posts_do_not_emit_triggers` |
| I15 | Known-occupant tell posts a stamped kind 9 with no trigger `--reply-to` and MUST NOT close a turn; unknown sender or `to=` MUST NOT post | `known_occupant_tell_posts_without_trigger_reply_to`, `occupant_tell_tag_routes_and_drops_scratch`, `unknown_tell_destination_does_not_post`, `unknown_named_sender_tell_does_not_post`, `disagreeing_tell_identity_does_not_post`, `missing_reply_to_acks_without_publish`, `occupant_tell_without_tag_posts_the_whole_body` |
| I16 | Occupant panes MUST be allocated with `herdr workspace create`, never `tab create` | `allocate_uses_workspace_create_not_tab_create` |
| I17 | One progress post per ask (D42): host-created after the hold and host-edited under the interval and cap, no `--mention`, kept on final, Buzz-deleted (kind 9005) on cancel, excluded from snapshots and ask Context, never relayed from the delivery handler | `progress_acks_without_publish`, `final_after_progress_discards_the_pending_body`, `progress_posts_once_after_hold_without_mention`, `later_bodies_coalesce_into_one_edit_after_the_interval`, `edit_cap_drops_later_bodies_with_one_notice`, `progress_body_is_trimmed_and_capped_at_the_protocol_ceiling`, `final_leaves_the_progress_post_up_and_drops_a_pending_edit`, `cancel_ends_progress_and_buzz_deletes_the_post`, `crash_after_create_dispatch_redelivers_the_same_id`, `flush_skips_relay_once_a_final_is_in_flight`, `cancel_during_the_final_retry_window_still_deletes_an_unaccepted_create`, `progress_once_a_final_is_in_flight_records_nothing`, `unaccepted_create_on_a_closed_turn_is_ended_and_leaves_the_scan`, `cancelled_progress_post_deleted`, `edit_replacement_deletes_the_old_progress_post`, `snapshot_progress_post_excluded`, `ask_context_excludes_progress_post`, `flow_11_progress_is_one_edited_post_then_a_final` |
| I18 | A host wake MUST carry a typed section, queue behind open work, and publish its final once through the normal stamped path without a fake reply or requester mention; migration MUST preserve existing trigger reply targets | `host_watch_fire_opens_a_normal_turn_with_a_typed_section`, `host_watch_fire_queues_behind_an_open_turn`, `host_wake_final_adds_only_body_derived_participant_mentions`, `existing_turns_table_gains_publish_claimed`, `repeated_migration_keeps_host_wake_publish_target_empty` |
| I19 | Watched-author matching MUST run in the host, record before wake, fire once per cooldown, ignore unwatched authors, exclude wake ids from mutation scope, and refresh author subscription scope when watches change | `watched_author_fires_once_per_cooldown_and_unwatched_author_does_not`, `watch_scope_lifetime_and_cancel_control_author_subscription`, `host_watch_fire_queues_behind_an_open_turn`, `subscription_refresh_replaces_all_dynamic_filters`, `subscription_refresh_gate_covers_scope_errors_and_idle` |
| I21 | A retryable publish failure MUST be resent on the host tick without a reconnect, reusing the prepared event id; abandoning after the retry bound MUST notice the operator and MUST NOT leave an ask turn open | `occupant_tell_timeout_is_retried_on_the_tick_with_the_same_id`, `ask_final_abandon_after_the_retry_bound_fails_the_turn`, `abandoned_tell_acks_without_sending_on_reconnect` |
| I22 | A tell body MUST publish whole and unaltered, and a refused tell MUST feed back to the occupant | `occupant_tell_publishes_any_text_including_angle_brackets`, `unpublishable_tell_feeds_back_to_the_occupant` |
| I23 | Unproven starts MUST retain their attempt and logical identity across retries and restart; completing a launch MUST atomically bind the session | `readiness_conflicts_and_host_restart_keep_one_start_identity`, `readiness_conflict_adopts_the_exact_recorded_identity`, `missing_start_report_does_not_authorize_a_replacement`, `proven_failed_start_continues_identity_and_retains_attempt_history`, `failed_binding_commit_leaves_launch_unsettled_and_recovers_without_starting_twice`, `reconciliation_refuses_a_conflicting_seat_without_adopting`, `reconciliation_recovers_a_ready_successor_after_an_adoption_receipt_is_lost` |
| I24 | Outbound occupant prose MUST notification-tag only uniquely named channel participants, keep readable labels, retain the requester first on replies, deduplicate in canonical order, and emit at most 50 `p` tags | `channel_message_deduplicates_and_caps_mentions_in_input_order`, `member_list_wins_and_profile_names_are_resolved`, `participant_mentions_use_longest_unique_alias_and_refuse_ambiguous_aliases`, `reply_retains_requester_and_adds_named_participant_mentions`, `resolved_mentions_deduplicate_with_requester_first_and_cap_at_fifty`, `known_occupant_tell_posts_without_trigger_reply_to`, `host_wake_final_adds_only_body_derived_participant_mentions`, `live_publishes_each_kind_and_dedups_a_redelivery` |
| I25 | A replacement occupant for an open turn MUST NOT be asked again and MUST NOT carry conversation memory; the bootstrap MUST point at `kelpie ask-info` for that ask; an ambiguous name/corpus match MUST refuse to choose | `gone_open_occupant_is_continued_without_a_second_ask`, `a_replacement_start_carries_no_conversation_memory`, `a_pane_still_holding_the_name_is_reclaimed_before_the_start`, `claimed_refuses_two_panes_holding_the_same_name_in_the_same_corpus`, `opening_an_alpha20_database_drops_the_backend_session_column` |
| I26 | A queued Turn whose dispatch fails past the bound MUST fail once, remove the marker, notice the operator with the reason, and record exactly one hardcoded requester notice replying to the trigger; a failed turn MUST NOT be dispatched again | `dispatch_failure_inside_the_bound_keeps_the_turn_queued`, `dispatch_failure_past_the_bound_fails_the_turn_and_posts_one_notice`, `dispatch_failure_stamps_once_and_fails_only_queued_turns` |

SPEC user-visible flows are the product matrix in `docs/testing.md`.

D66 amends D57 and narrows I5: an empty requester list admits relay members; a
non-empty list is exact in addition to the operator.
`requester_allowlist_defaults_to_everyone_and_normalizes_keys` and
`requester_policy_is_per_bot_and_peers_still_require_a_mention` prove admission.
`requester_stamp_uses_host_identity_not_request_text` proves prefix origin.
`queued_requester_checks_cover_legacy_rows_revocation_and_missing_identity`
proves queued dispatch fails closed; `local_relay_contract_before_synthetic_occupant`
proves refusal, operator/peer stamps and peer queue/restart on an isolated relay.
`observed_author_fallback_is_explicitly_shared` proves uncertain membership can
never render private. `mechanical_output_scrub_refuses_secrets_and_operator_paths_privately`
and `scrubbed_progress_and_tell_feed_back_without_publishing` prove the three
output refusals do not publish or echo the body, and that each refusal feeds back
to the occupant. Finals fail terminally; progress and tells are acknowledged.

The host contract and snapshot blocks MUST be idempotent and MUST preserve
author-owned text and `AGENTS.md` (D55):
`startup_contract_preserves_author_text_and_refreshes_both_blocks_idempotently`,
`startup_creates_missing_blocks_and_fills_the_shipped_template`, and
`malformed_startup_markers_leave_the_file_untouched`.
D70's watch-grammar text is asserted by
`startup_contract_preserves_author_text_and_refreshes_both_blocks_idempotently`.
D72's search grammar and empty-versus-failed rendering are asserted by
that contract test, `empty_search_is_not_rendered_as_failure`,
`failed_search_is_not_rendered_as_empty`,
`search_runs_without_operator_env`, and
`local_relay_contract_before_synthetic_occupant`.
The lookup socket MUST NOT be reachable by another user:
`the_lookup_socket_is_not_reachable_by_another_user`.
The advice path MUST follow the running installation:
`conduct_path_follows_the_running_installation_not_the_build_directory`.

Secrets (D29): wrap binaries with `envchain NAMESPACE cmd`. Do not add
`--envchain` flags.
