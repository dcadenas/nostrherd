...
| I25 | A replacement occupant for an open turn MUST NOT be asked again; the bootstrap MUST point at `kelpie ask-info` for that ask, and a backend session Herdr reports on the new pane MUST be stored without wiping a known token | `gone_open_occupant_is_continued_without_a_second_ask`, `a_gone_open_occupant_replays_the_stored_backend_session`, `first_start_does_not_record_a_backend_session_when_herdr_has_not_reported_one`, `first_start_records_the_backend_session_herdr_reports_on_the_new_pane`, `recorded_session_reads_pane_get_agent_session_value` |
