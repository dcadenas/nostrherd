...
| I11 | Binaries MUST read `BUZZ_PRIVATE_KEY` and `BUZZ_RELAY_URL` when set, and MUST NOT take `--envchain` | `operator_env_reads_buzz_private_key_and_relay_url`, `botcli_reads_buzz_private_key_and_relay_url`, `botserver_parser_rejects_envchain`, `parser_requires_send_and_exactly_one_body_source` |
