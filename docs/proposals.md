...
the ask is owned by waiter `nostrherd` (D5). Both proposals need the
...
nostrherd already has that substrate, since the host is the silent
...
- **Static text, no host change.** An occupant runs
  `kelpie tell nostrherd --every 5m` with fixed text. Each firing is an
...
- **Fresh status, no host change.** An occupant schedules a repeating
  ...
  tells `nostrherd` with the result. The host sees an ordinary tell.
...
byte-identical "When a Kelpie ask arrives from nostrherd" section in
...
this session (`kelpie tell nostrherd --due-in 10m` at
...
- Tier 1 (contract, must never drift): a second host-managed marker
  block, `nostrherd-contract`, upserted into corpus `startup.md` on
  start with the same idempotent rewrite the snapshot block already
  uses (`crates/nostrherd/src/snapshot.rs`, `upsert_startup_block`).
  ...
  - You MAY `kelpie tell nostrherd` for a bot-initiated post (D38).
  ...
  - Do not reply without a nostrherd ask. Context sections are
...
- Tier 2 (advice, may vary per bot): the `bot-conduct` guidance
  (`skills/bot-conduct/SKILL.md`), shipped inside the nostrherd
