...
impl KelpieClient {
    ...
    /// Return the identity to continue for a public name, if one exists.
    ///
    ...
    /// Schedule the occupant's periodic context renew (D27).
    ///
    /// # Errors
    ///
    /// Returns an error when Kelpie cannot arm the schedule or answers
    /// without a renew id.
