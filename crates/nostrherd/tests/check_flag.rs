...
fn temp_path(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "nostrherd-check-{label}-{}-{timestamp}-{sequence}",
        std::process::id()
    ))
}
...
fn check_exits_zero_after_loading_config_and_database() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_nostrherd"))
        .args([
            "--config",
...
fn check_exits_nonzero_when_config_is_missing() {
    ...
    assert!(message.contains("no bot registry at"));
    assert!(message.contains(config.to_str().expect("utf8")));
    assert!(message.contains("nostrherd init <dir> --config"));
